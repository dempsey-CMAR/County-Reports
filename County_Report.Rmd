...
custom_DO_lims <- c("Aberdeen", "Piper Lake", "Hourglass Lake", "Sissiboo",
                    "5008") 
...
for(i in seq_along(waterbodies)){
  ...
  # waterbody table
  table <- write_report_table(
    waterbody_dat, keep.waterbody = FALSE, keep.units = DO.mgL
  )
...
kable(
  params$doc.hist, align = "ccl", caption = glue("Table {i}: Document history.")
)
